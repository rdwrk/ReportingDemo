...
namespace Reporting.WebDemo.Pages
{
    ...
    public class SalesReportModel : PageModel
    {
        ...
        public SalesReportModel(ReportService svc) => _svc = svc;
        ...
        public IActionResult OnPost(string dateFrom, string dateTo, string region, string preparedBy)
        {
            ...
            PdfUrl     = $"/reports/stream?report=sales&inline=true&dateFrom={dateFrom}&dateTo={dateTo}&filter={region}&preparedBy={Uri.EscapeDataString(preparedBy ?? "")}";
            return Page();
        }
    }
}
